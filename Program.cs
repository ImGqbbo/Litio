    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                new Task(async () =>
                {
                    await BanQueue.StartAsync(client);
                }).Start();
                ...
                Console.WriteLine(" [Litio] Ready!");
                Console.Write(" [Litio] Bot token? [Y/N] ");
                string botToken = Console.ReadLine();
                Console.Write(" [Litio] Insert token: ");
                string token = Console.ReadLine();

                Console.WriteLine(" [Litio] Token set as: " + token);
                Console.WriteLine(" [Litio] Logging in...");
                client.Login(botToken.ToLower() == "y" ? "Bot " + token : token);
                Console.WriteLine(" [Litio] Sent login request.");

                Console.ReadLine();
            }
            catch (InvalidTokenException)
            {
                Console.WriteLine(" [Litio] Invalid token, press enter to exit.");
                Console.ReadLine();
            }
        }
                ...
                foreach (var guild in args.Guilds)
                {
                    if (!Raiders.ContainsKey(guild.Id))
                    {
                        ...
                        if (!Utils.Guilds.Select(x => x.GuildId).Contains(guild.Id.ToString()))
                        {
                            ...
                            Utils.Guilds.Add(new LitioGuild() { TimeoutDuration = new TimeSpan(0, 30, 0), PunishmentType = LitioPunishment.Ban, GuildId = guild.Id.ToString(), Toggled = true });
        ...
        private static void Client_OnUserJoinedGuild(DiscordSocketClient client, GuildMemberEventArgs args)
        {
            try
            {
                ...
                if (raiders.Count > 0)
                {
                    raiders.RemoveAll(m => m.JoinedAt < (DateTime.Now - new TimeSpan(0, 0, 4)) - JoinExpiration);
                }
        ...
        private static void Client_OnThreadCreated(DiscordSocketClient client, ThreadEventArgs args)
        {
            try
            {
                ...
                {
                    threadRaiders.RemoveAll(m => m.Metadata.CreatedAt < (DateTime.Now - new TimeSpan(0, 0, 4)) - ThreadExpiration);
                }
        ...
        private static void Client_OnMessageReceived(DiscordSocketClient client, MessageEventArgs args)
        {
            try
            {
                if (!string.IsNullOrEmpty(args.Message.Content))
                {
                    ...
                    if (messages.Count > 0)
                    {
                        messages.RemoveAll(m => m.SentAt < DateTime.Now - new TimeSpan(0, 0, 4) - MessageExpiration);
                    }
